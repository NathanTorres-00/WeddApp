...
import { toggleFavorite } from '../lib/vendors';
import { ContactVendorModal } from './vendors/ContactVendorModal';
...
export const VendorCard: React.FC<VendorCardProps> = ({
  ...
  const handleFavoriteClick = async () => {
    try {
      const isNowFavorite = await toggleFavorite(vendor.id);
      onFavoriteToggle();
          ...
          <div className="flex items-center space-x-3 pt-2">
            ...
            <motion.button
              ...
              className={`p-3 rounded-xl hover:bg-gray-100 ${
                vendor.isFavorite ? 'text-red-500' : 'text-gray-600 hover:text-red-500'
              }`}
            >
              <Heart className={`w-6 h-6 ${vendor.isFavorite ? 'fill-current' : ''}`} />
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
